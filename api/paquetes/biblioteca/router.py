...
async def get_favoritos(user: dict = Depends(get_current_user)):
...
async def add_favorito(fact_id: int, user: dict = Depends(get_current_user)):
    user_id = user["record"]["id"]
...
async def remove_favorito(fact_id: int, user: dict = Depends(get_current_user)):
    user_id = user["record"]["id"]
    _assert_fact_exists(fact_id)
...
async def add_historial(fact_id: int, user: dict = Depends(get_current_user)):
    user_id = user["record"]["id"]
    _assert_fact_exists(fact_id)
