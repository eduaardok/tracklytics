...
## Implementation Notes (resuelto en /opsx:apply)

Al implementar, no existía aún ninguna fuente para el directorio de partners (ni colección de PocketBase ni tabla `DIM_PARTNER` en ClickHouse) — CU-T03 no está implementada en este repo. Para no bloquear esta capability, se creó (con confirmación explícita del usuario) el mínimo sustrato necesario, documentado como un placeholder hasta que CU-T03 se implemente:

- **Colección PocketBase `partners`** (`pb_init.py`): campos `nombre`, `api_key`, `tier`, `estado`, `fecha_expiracion` (opcional), `created`. Sin `listRule`/`viewRule`/etc. — solo el superusuario de PocketBase puede leerla/escribirla, ya que no existe sesión de "usuario partner". FastAPI se autentica contra ella usando credenciales de superusuario (`PB_ADMIN_EMAIL`/`PB_ADMIN_PASSWORD`), no un flujo de usuario final.
- **Tabla ClickHouse `LOG_LLAMADAS_PARTNER`** (`init_clickhouse.py`): `log_id`, `partner_id`, `api_key_used`, `endpoint`, `tier_usado`, `resultado` (enum success/auth_rejected/tier_rejected/error), `registros`, `duracion_ms`, `timestamp`. Es el log operativo descrito en la sección "Registro de llamadas" — explícitamente NO es `FACT_INTEGRACION_PARTNER`.
- La API valida el formato de la llave de API con una regex (`^[A-Za-z0-9_-]{16,128}$`) antes de interpolarla en el filtro de PocketBase, dado que el valor proviene de un llamador externo no autenticado (a diferencia del resto del proyecto, donde los filtros de PocketBase se construyen con valores ya confiables).
- El registro de llamadas se implementó como middleware HTTP (`paquetes/partners/logging_mw.py`) scoped a `/partners/v1/*`, en vez de una llamada explícita por endpoint — captura tanto los rechazos de autenticación/tier (lanzados desde la dependencia, antes del handler) como las respuestas exitosas, sin duplicar lógica de logging en cada endpoint.
