...
El sistema SHALL resolver una portada real para artistas y álbumes del catálogo licenciado base mediante búsqueda en un directorio musical externo público, almacenando la imagen resuelta. El sistema SHALL usar un reemplazo visual generado localmente, sin ninguna llamada externa, cuando no exista una portada resuelta o el contenido no pertenezca al catálogo licenciado base.

#### Scenario: Portada resuelta exitosamente
- **WHEN** el proceso de resolución de portadas encuentra una imagen para un artista o álbum del catálogo licenciado base
...
- **WHEN** el proceso de resolución de portadas no encuentra una imagen para un artista o álbum, o el contenido no pertenece al catálogo licenciado base
...
El sistema SHALL permitir reproducir audio real de un track mediante búsqueda por texto en un directorio de video externo, ejecutada desde el propio cliente. El sistema SHALL deshabilitar el control de reproducción o mostrar un estado de no disponible para ese track, sin afectar el resto de la interfaz, cuando no haya conexión o no se encuentre un resultado.
...
#### Scenario: Reproducción real no disponible
- **WHEN** un Usuario B2C intenta reproducir un track y no hay conexión o no se encuentra un resultado en el directorio externo
- **THEN** el sistema deshabilita el control de reproducción de ese track o muestra un estado de no disponible, sin afectar el resto de la interfaz
