...
- `solicitudes_recibidas/aprobadas/rechazadas` + tiempo de resolución:
  ...
  DIM_ESTADO_REVISION), agregado por semana de `fecha_subida`.
- `licencias_activas` por territorio: DIM_LICENCIA (estado='activa') unido
  ...
  de altas/bajas por semana), repetido igual en todos los períodos de la
  ventana. Documentado: no es una serie histórica real semana a semana,
...
COLUMNS = [
    "periodo", "territorio", "solicitudes_recibidas", "aprobadas", "rechazadas",
    "tasa_aprobacion", "tiempo_promedio_resolucion_h", "licencias_activas",
    "tracks_cubiertos", "cobertura_pct", "es_estimado",
]


def run_gold_contenido() -> None:
    t0 = time.time()
    ...
    solicitudes = {r["periodo"]: r for r in catalog.query(
        f"""
        SELECT {periodo_sql('fecha_subida')} AS periodo,
               count() AS total,
        ...
        FROM FACT_SUBIDA_TRACK WHERE fecha_subida >= now() - INTERVAL 90 DAY GROUP BY periodo
        """
    ).named_results()}
    ...
    for periodo in periodos:
        ...
        for territorio in territorios:
            ...
            rows.append((periodo, territorio, *fila_general[:5], lic_n, cub_n, cobertura_pct, fila_general[5]))
