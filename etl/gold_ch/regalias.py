...
solo 3 contratos y sus liquidaciones caen mayormente fuera de la ventana de
12 semanas recientes (fechas de prueba entre 2022-2028) — se usa
`fecha_calculo` (cuándo se registró la liquidación) para bucketing por
período; las semanas de la ventana sin liquidación real se rellenan con
demo usando los MISMOS 3 contratos/sellos reales (no se inventan contratos
nuevos), marcadas `es_estimado=1`.
"""
...
COLUMNS = [
    "periodo", "contrato_id", "sello", "tipo_split", "monto_liquidado",
    "reproducciones_periodo", "porcentaje_aplicado", "es_estimado",
]


def run_gold_regalias() -> None:
    t0 = time.time()
    ...
    reales = list(catalog.query(
        f"""
        SELECT {periodo_sql('fecha_calculo')} AS periodo, contrato_id, tipo_rightsholder,
               sum(monto) AS monto, sum(streams_periodo) AS streams
        FROM FACT_LIQUIDACION_REGALIA
        WHERE fecha_calculo >= now() - INTERVAL 90 DAY
        GROUP BY periodo, contrato_id, tipo_rightsholder
        """
    ).named_results())
    ...
    for r in reales:
        rows.append((
            r["periodo"], r["contrato_id"], sello_por_contrato.get(r["contrato_id"], ""),
    ...
    for periodo in periodos:
        for c in contratos:
            ...
            rows.append((
                periodo, c["contrato_id"], c["sello"], "sello",
